...
import type * as User from "./types/account"
...
export function getWallets(data: IListRequest) {
  return request<IResponse<any>>({
    url: "account_wallet/my",
    method: "post",
    data
  })
}

export function exchangeAmountApi(amount: number) {
  return request<IResponse<any>>({
    url: "account/exchange_amount",
    method: "post",
    data: {
      amount
    }
