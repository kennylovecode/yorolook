...
<template>
  <div :class="{ 'simple-mode': props.isCollapse && !isTop, 'first-level': props.isFirstLevel }">
    <SidebarItemLink v-if="theOnlyOneChild?.meta" :to="resolvePath(theOnlyOneChild.path.split(':')[0])">
      <el-menu-item>
        <SvgIcon v-if="theOnlyOneChild.meta.svgIcon && !isTop" :name="theOnlyOneChild.meta.svgIcon" />
        <component
          v-else-if="theOnlyOneChild.meta.elIcon && !isTop"
          :is="theOnlyOneChild.meta.elIcon"
          class="el-icon"
        />
        <template #title>
          <span style="height: 18px; line-height: 18px">{{ theOnlyOneChild.meta?.title }}</span>
        </template>
      </el-menu-item>
    </SidebarItemLink>
