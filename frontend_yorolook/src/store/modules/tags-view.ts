...
export const useTagsViewStore = defineStore("tags-view", () => {
  ...
  const visitedViews = ref<TagView[]>(preference.value.cacheTagsView ? getVisitedViews() : [])
  const cachedViews = ref<string[]>(preference.value.cacheTagsView ? getCachedViews() : [])
  ...
  const editTagTitle = (view: TagView, title: string) => {
    if (typeof view.fullPath !== "string") return
    const index = visitedViews.value.findIndex((x) => x.fullPath === view.fullPath)
    if (!visitedViews.value[index]) return
    visitedViews.value[index].meta!.title = title
  }

  //#region add
  const addVisitedView = (view: TagView) => {
    // 检查是否已经存在相同的 visitedView
    const index = visitedViews.value.findIndex((v) => v.fullPath?.replace(/\?.*$/, '') === view.fullPath?.replace(/\?.*$/, ''))

    if (index !== -1) {
      // 防止 query 参数丢失
      // visitedViews.value[index].fullPath !== view.fullPath && (visitedViews.value[index] = { ...view })
      visitedViews.value[index].fullPath !== view.fullPath && (visitedViews.value[index].fullPath = view.fullPath)
    } else {
      // 添加新的 visitedView
