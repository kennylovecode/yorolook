...
<template>
  <div class="app-container">
    <el-row v-if="articleModel.brand?.id" class="publish-header">
      <el-col :xl="16" :lg="16" :md="16" :sm="16" :xs="24">
        <div class="logo">
          <el-avatar :size="48" shape="circle" src="articleModel.brand?.logo" fit="fill" @error="() => true">
            {{ articleModel.brand?.title }}
          </el-avatar>
        </div>
        <div class="title">
          <span>{{ articleModel.brand?.sub_title }}</span>
          <span>{{ articleModel.brand?.level }}</span>
        </div>
        <div class="keywords">
          <el-tag type="info" size="small" effect="dark" plain
            >城市-{{ articleModel.brand?.country + articleModel.brand?.city }}</el-tag
          >
          <el-tag type="info" v-if="articleModel.brand?.catalog" size="small" effect="dark" plain
            >主营-{{ articleModel.brand?.catalog }}</el-tag
          >
          <el-tag type="info" size="small" effect="dark" plain>{{ articleModel.brand?.type }}</el-tag>
        </div>
      </el-col>
      <el-col :xl="8" :lg="8" :md="8" :sm="8" :xs="24">
        <el-button type="primary" size="default" @click="openBrandHome" plain>品牌主页</el-button>
    ...
    <purchase-panel v-if="articleModel.id" :article="articleModel" />
  </div>
</template>
...
onMounted(async () => {
  ...
  tagsViewStore.editTagTitle(route, `${articleModel.value.title} - 产品详情`)
})
</script>
