...
.bg-banner {
  background: url(/public/brand-banner-93c3fa22.jpg) no-repeat;
  width: 100%;
  height: 300px;
...
<template>
  ...
  <div class="app-container">
    <catalog-selector
      :key="'hello_' + catalogIDPath"
      :data-filter="filterCatalog"
      display-key="title"
      value-key="id"
      :show-icon="false"
      :show-child="false"
      :ratio="4 / 3"
      :id="catalogIDPath"
      @on-change="onCatalogChange"
    />
    <multiple-selector
      :key="brandType?.options.length + searchQuery.type!.length"
      :data="brandType?.options"
      text="类型"
      :show-child="false"
      display-key="text"
      value-key="text"
    ...
    <multiple-selector
      ...
      :show-child="false"
      :simple="true"
      v-model="searchQuery.initial"
      @on-change="load(true)"
    />
    <filter-area
      :key="searchQuery.attributes"
      :defaults="searchQuery.attributes"
      name="name"
      label="title"
      value="title"
...
import { getList } from "@/api/brand/index"
import { Brand, ListRequest } from "@/api/brand/types"
import { ref, onMounted, onBeforeMount, computed, reactive, shallowReactive } from "vue"
...
import { deleteById } from "@/api/brand/index"
